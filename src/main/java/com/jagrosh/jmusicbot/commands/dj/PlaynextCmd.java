...
public class PlaynextCmd extends DJCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyWarning("請包含歌曲名稱或URL!");
            return;
        }
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track)
        {
            ...
            {
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" 這首歌曲 (**"+track.getInfo().title+"**) 長度已超過最大允許值: `"
            ...
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()+" 添加 **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0?"開始播放":" to the queue at position "+pos));
        ...
        {
            if(ytsearch)
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" 沒有找到結果 `"+event.getArgs()+"`.")).queue();
            else
