...
public class SettcCmd extends AdminCommand 
{
    ...
    protected void execute(CommandEvent event) 
    {
        ...
        {
            event.reply(event.getClient().getError()+" 請包含文字頻道或NONE");
            return;
        }
        ...
        if(event.getArgs().equalsIgnoreCase("none"))
        {
            ...
            event.reply(event.getClient().getSuccess()+" 音樂指令現在可以在任何文字頻道內使用");
        }
        else
        {
            List<TextChannel> list = FinderUtil.findTextChannels(event.getArgs(), event.getGuild());
            if(list.isEmpty())
                event.reply(event.getClient().getWarning()+" 沒有找到符合的文字頻道 \""+event.getArgs()+"\"");
            else if (list.size()>1)
